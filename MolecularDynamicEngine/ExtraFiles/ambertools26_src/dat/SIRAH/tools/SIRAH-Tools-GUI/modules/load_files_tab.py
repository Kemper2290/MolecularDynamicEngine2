...
This module provides the file-loading interface for the SIRAH Tools GUI.
It creates the "Load Files" tab, allowing users to configure the working
environment and load the molecular system required for trajectory analysis.

The module manages the selection of topology and trajectory files,
initializes the working directory structure, and validates that the
required visualization backend (VMD) is available in the system path.

Features:
    - Select and manage the working directory for analysis outputs.
    - Load topology and trajectory files for molecular simulations.
    - Load an optional reference structure for comparative analyses.
    - Validate topology and trajectory file formats before system loading.
    - Initialize and maintain a shared analysis state object.
    - Open the loaded system directly in VMD for visualization.
    - Configure basic simulation parameters (time step and frame spacing).
    - Display tooltips and status indicators for user guidance.
    - Provide system reset functionality for reinitializing the workflow.
...
Usage:
    This module is integrated into the SIRAH Tools GUI and is responsible
    for initializing the analysis environment before any analysis modules
    are executed.

    The function `create_load_files_tab()` builds the complete interface
    for loading simulation files and returns a reset function to restore
    the tab to its initial state.
...
def load_topology(state: AnalysisState, button: ttk.Button, label: ttk.Label,
    ...
    if file_path:
        ...
        # Reset status to "No system loaded" in red
...
def create_load_files_tab(tab: ttk.Frame, state: AnalysisState, reset_callback: callable) -> callable:
    ...
    # Label for system status (default "No system loaded" in red)
