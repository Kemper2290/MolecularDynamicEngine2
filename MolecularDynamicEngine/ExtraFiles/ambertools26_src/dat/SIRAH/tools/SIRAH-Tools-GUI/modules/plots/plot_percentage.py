"""
Contact Percentage Map Viewer

This module provides a graphical interface for visualizing contact
percentage maps generated during SIRAH contact analysis. It reads
the `percentage_*.dat` files produced by the contacts analysis
workflow and reconstructs a residue–residue contact matrix based
on the percentage of frames in which each contact occurs.

The application integrates a Tkinter-based GUI with Matplotlib
visualization tools, allowing users to explore the contact map,
change colormaps interactively, and export figures with
customizable resolution.

Features:
    - Automatically locate `percentage_*.dat` files inside the
      Contacts analysis directory.
    - Read residue–residue contact percentages from analysis output.
    - Construct a symmetric contact matrix from percentage data.
    - Display the contact map in an interactive Tkinter window.
    - Allow dynamic colormap selection using a dropdown menu.
    - Export contact maps with user-defined DPI.
...
Usage:
    This module is used internally by the SIRAH Tools GUI
    to visualize contact percentage maps generated during
    the contacts analysis workflow.

    The function `plot_percentage_file(state)` searches for
    percentage files in the working directory and launches
