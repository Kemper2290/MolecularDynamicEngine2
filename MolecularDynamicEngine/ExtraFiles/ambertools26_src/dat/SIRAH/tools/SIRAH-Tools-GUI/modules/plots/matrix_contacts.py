"""
Contact Matrix Plotter

This script provides an interactive graphical application for generating
and visualizing residue contact matrices from contact analysis data.
It reads the files produced during contact analysis and reconstructs
the contact matrix, allowing users to explore and export the resulting
contact maps.

The application integrates a Tkinter-based GUI with Matplotlib and
Seaborn plotting capabilities, enabling interactive visualization,
colormap selection, and figure export with customizable resolution
and format.

Features:
    - Read contact matrix metadata from `matrix_length.txt`.
    - Read contact percentage data from `percentage_*.dat`.
    - Automatically determine whether the contact matrix is symmetric
      based on matrix dimensions.
    - Generate symmetric contact maps (e.g., protein vs protein).
    - Generate asymmetric contact maps (e.g., protein vs ligand or
      protein vs lipid).
    - Display the contact map in an interactive GUI window.
    - Allow dynamic selection of Matplotlib colormaps.
    - Provide navigation tools for zooming and panning.
    - Export contact maps in multiple formats with user-defined DPI.
    - Save figures in formats including PNG, PDF, SVG, EPS, JPG, and TIFF.
    - Log execution details and errors to a log file.
...
Usage:
    Run the script from the command line providing the required files:

        python matrix_contacts.py matrix_length.txt percentage_data.dat

    Where:
        matrix_length.txt  : File describing the contact matrix dimensions
                             and residue selections.
        percentage_data.dat: File containing contact percentages between
                             residue pairs.
