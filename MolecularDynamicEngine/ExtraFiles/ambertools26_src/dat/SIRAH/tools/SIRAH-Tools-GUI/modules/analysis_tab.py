...
This module provides the basic analysis workflow for the SIRAH Tools GUI.
It creates the "Analysis" tab within the graphical interface and allows
users to configure and execute several trajectory analyses using VMD and
custom TCL scripts.

The module integrates GUI controls, analysis execution, data post-processing,
plot generation, and optional report creation.

Features:
    - Configure atom selections using VMD selection syntax.
    - Perform basic trajectory analyses:
        • RMSD (Root Mean Square Deviation)
        • RMSF (Root Mean Square Fluctuation)
        • Radius of Gyration
    - Perform advanced analyses:
        • SASA (Solvent Accessible Surface Area)
        • Distance between atom selections
        • Radial Distribution Function (RDF)
        • Contact surface area
    - Execute the VMD analysis backend using the `sirah_analysis.tcl` script.
    - Display real-time VMD output inside the GUI.
    - Automatically generate plots from analysis output files.
    - Optionally generate a PDF report summarizing results.
    - Convert RMSF values into PDB B-factor fields for visualization.
...
Usage:
    This module is intended to be integrated within the SIRAH Tools GUI
    application and is initialized through the main GUI notebook interface.

    The function `create_analysis_tab()` builds the complete analysis tab
    and returns a reset function to restore its default state.
...
def analyze(state) -> None:
    ...
    if existing_files:
        ...
        # *** ATTENTION: This function goes INSIDE the if statement, so it accesses the message ***
        def ask_overwrite():
            overwrite_decision["value"] = messagebox.askyesno("Overwrite Files?", message)
            decision_event.set()

        state.root.after(0, ask_overwrite)
        decision_event.wait()  # Wait until the user responds
