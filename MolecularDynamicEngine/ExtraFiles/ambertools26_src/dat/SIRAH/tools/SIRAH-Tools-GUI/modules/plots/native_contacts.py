"""
Native Contacts Plotter

This script provides an interactive graphical application for visualizing
native contact analysis results obtained from molecular dynamics simulations.
It reads contact timeline data and generates a time-resolved plot displaying
contact conservation, accuracy, and the total number of contacts during the
simulation.

The application integrates a Tkinter-based GUI with Matplotlib plotting
capabilities, allowing users to explore the temporal evolution of contacts
and export figures with customizable resolution and format.

Features:
    - Load native contact timeline data from analysis output files.
    - Convert simulation frames to time using a user-defined scaling factor.
    - Plot conservation and accuracy percentages over time.
    - Display the number of contacts on a secondary y-axis.
    - Provide an interactive GUI window with navigation toolbar.
    - Allow exporting figures with customizable DPI and output format.
    - Automatically adapt window size based on screen resolution.
...
Usage:
    Run the script from the command line providing the input data file:

        python native_contacts.py timeline_contacts.dat --time_factor 0.02

    Where:
        timeline_contacts.dat : File containing contact conservation,
                                accuracy, and contact count over frames.
        --time_factor         : Factor to convert frames into simulation time
                                (e.g., microseconds).
