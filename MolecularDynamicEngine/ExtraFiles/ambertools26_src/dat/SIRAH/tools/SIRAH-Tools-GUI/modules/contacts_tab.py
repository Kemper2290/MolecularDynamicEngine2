...
This module provides the contact analysis interface for the SIRAH Tools GUI.
It creates the "Contacts" tab and allows users to compute intermolecular or
intra-molecular contacts using VMD together with custom TCL analysis scripts.

The module integrates GUI controls, execution of VMD-based calculations,
and post-analysis plotting utilities.

Features:
    - Configure atom selections using VMD selection syntax.
    - Define frame skipping and contact cutoff distance.
    - Execute contact analysis using the `contacts_distance.tcl` script.
    - Optionally calculate the full distance matrix between selections.
    - Display real-time VMD output within the GUI.
    - Generate contact analysis datasets including:
        • Contact timelines
        • Contact percentages
        • Contact lengths
        • Distance-by-frame data
    - Perform additional post-processing analyses:
        • Native contacts conservation
        • Contact maps
        • Distance maps by frame
    - Manage execution in background threads to avoid blocking the GUI.
    - Allow users to stop running VMD processes safely.
    - Log operations and errors for debugging and reproducibility.
...
Usage:
    ...
    for performing contact-based structural analyses after a molecular
    system has been loaded in the "Load Files" tab.

    The function `create_contacts_tab()` builds the graphical interface
    and connects user inputs to the VMD analysis backend.
