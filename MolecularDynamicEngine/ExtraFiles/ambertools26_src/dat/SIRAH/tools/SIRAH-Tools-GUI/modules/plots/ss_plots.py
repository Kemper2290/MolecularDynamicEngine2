"""
Secondary Structure Plotting Tool

This script generates publication-quality plots of secondary structure
assignments obtained from molecular dynamics simulations using the
SIRAH analysis workflow. It reads the output files produced by the
`sirah_ss.tcl` script and provides several visualization modes to
analyze secondary structure evolution.

The script supports plotting secondary structure as a function of
simulation time, per-residue secondary structure distributions, and
secondary structure matrices displayed as heatmaps.

Features:
    - Plot secondary structure percentage vs time using `ss_by_frame.xvg`.
    - Plot secondary structure distribution per residue using `ss_by_res.xvg`.
    - Generate heatmap visualization of secondary structure evolution
      using `ss.mtx`.
    - Automatically detect which secondary structure types are present
      (H, E, C) and generate the corresponding legend.
    - Display progress during processing using a progress bar.
    - Save figures in high resolution for publication-quality output.
...
Usage:
    Run the script from the command line providing the input file
    and desired plot type:

        python ss_plots.py -i ss.mtx -t mtx
        python ss_plots.py -i ss_by_frame.xvg -t frame
        python ss_plots.py -i ss_by_res.xvg -t res

    Optional parameters allow customization of output image size,
    resolution, colors, axis formatting, and titles.
