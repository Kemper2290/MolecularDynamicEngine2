...
This module provides the secondary structure (SS) analysis interface for the
SIRAH Tools GUI. It creates the "SS Analysis" tab and allows users to compute
secondary structure assignments from molecular dynamics trajectories using
VMD together with the `sirah_ss.tcl` analysis script.

The module integrates GUI controls, execution of VMD-based calculations,
real-time display of analysis output, and automated plotting utilities.

Features:
    - Configure analysis parameters including:
        • first and last frame
        • atom selection using VMD selection syntax
        • frame sampling interval (each)
    - Optionally compute Psi/Phi dihedral angles for Ramachandran analysis.
    - Execute the secondary structure analysis using the `sirah_ss.tcl` script.
    - Display real-time VMD output within the GUI.
    - Automatically generate secondary structure output files including:
        • ss_by_frame.xvg  (secondary structure vs time)
        • ss_by_res.xvg    (secondary structure per residue)
        • ss_global.xvg    (global secondary structure statistics)
        • ss.mtx           (secondary structure matrix)
        • psi.mtx / phi.mtx (optional dihedral angle matrices)
    - Allow users to customize output file names.
    - Generate graphical plots from analysis results using `ss_plots.py`:
        • secondary structure heatmap
        • secondary structure vs time
        • secondary structure per residue
    - Provide a standalone Psi/Phi analysis using `ramach.py`.
...
Usage:
    This module is integrated into the SIRAH Tools GUI and performs
    secondary structure analysis after a molecular system has been loaded
    in the "Load Files" tab.

    The function `create_ss_analysis_tab()` builds the graphical interface
    and connects user inputs with the VMD-based secondary structure analysis
    backend.
