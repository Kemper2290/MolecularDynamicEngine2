...
This module provides the backmapping interface for the SIRAH Tools GUI.
It creates the "Backmapping" tab and allows users to reconstruct
all-atom structures from SIRAH coarse-grained trajectories using VMD
and AmberTools.

The module integrates GUI controls, execution of VMD-based backmapping
scripts, monitoring of the backmapping process, and visualization of
the generated atomistic structures.

Features:
    - Verify the AMBERHOME environment variable.
    - Detect AmberTools installation automatically if AMBERHOME is not set.
    - Configure basic backmapping parameters including:
        • first and last frame
        • frame sampling interval (each)
        • frame selection list
        • output file name
    - Provide advanced simulation options:
        • disable minimization
        • enable CUDA acceleration
        • enable GBSA implicit solvent
        • configure cutoff distance
        • configure MPI processes
        • configure minimization cycles (maxcyc, ncyc)
    - Execute the backmapping workflow using:
        • `backmapping.tcl`
        • `sirah_vmdtk.tcl`
    - Run VMD in text mode to perform the reconstruction.
    - Display real-time output of the backmapping process within the GUI.
    - Allow users to stop the running process safely.
    - Generate output files including:
        • reconstructed PDB structure
    - Open the resulting backmapped structure directly in VMD.
...
Usage:
    ...
    atomistic reconstruction (backmapping) after a molecular system
    has been loaded in the "Load Files" tab.

    The function `create_backmapping_tab()` builds the graphical
    interface and connects user inputs to the VMD-based backmapping
    backend.
