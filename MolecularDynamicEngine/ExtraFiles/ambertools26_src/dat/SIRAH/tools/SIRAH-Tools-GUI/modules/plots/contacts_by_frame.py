"""
Contacts Heatmap Viewer

This script provides an interactive graphical viewer for visualizing
distance/contact matrices generated from molecular dynamics contact
analysis. The application loads frame-by-frame distance data and
displays it as an interactive heatmap that can be explored over time.

The viewer integrates a Tkinter-based GUI with Matplotlib plotting
capabilities, allowing users to navigate through simulation frames,
adjust visualization parameters, and export figures or animations.

Features:
    - Load distance/contact data files generated during contact analysis.
    - Display frame-by-frame heatmaps of distance matrices.
    - Navigate through trajectory frames using an interactive slider.
    - Automatically scale the colorbar using global minimum and maximum values.
    - Change the colormap dynamically.
    - Export heatmap figures with user-defined DPI and format.
    - Generate animated GIFs showing the evolution of contacts over time.
    - Save individual frames used for animation.
