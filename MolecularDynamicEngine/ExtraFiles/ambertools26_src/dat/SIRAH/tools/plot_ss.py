"""
SIRAH Secondary Structure Plotter
=================================

Author: Andres Ballesteros & Lucianna Silva
Version: 1.0
Date: September 2023

Description
-----------
This script generates publication-quality plots from the secondary structure
analysis files produced by the SIRAH tool `sirah_ss`. It supports visualization
of secondary structure data as:

    1. Time evolution of secondary structure percentages
    2. Secondary structure distribution per residue
    3. Secondary structure heatmap (residue vs time)

The script reads the following output files generated by `sirah_ss`:

    ss_by_frame.xvg   -> Secondary structure percentage vs simulation time
    ss_by_res.xvg     -> Secondary structure percentage per residue
    ss.mtx            -> Secondary structure matrix (residue vs time)

Secondary structure states are represented as:

    H : Alpha helix
    E : Beta sheet
    C : Coil

The script can generate line plots, stacked bar plots, or heatmaps depending
on the selected input file or plot type.

Example usage
-------------

Plot secondary structure evolution vs time:

    python plot_ss.py -i ss_by_frame.xvg

Plot secondary structure distribution per residue:

    python plot_ss.py -i ss_by_res.xvg

Plot secondary structure heatmap:

    python plot_ss.py -i ss.mtx

Specify time units (default microseconds):

    python plot_ss.py -i ss_by_frame.xvg -tu ns

Custom colors:

    python plot_ss.py -i ss.mtx -H purple -E gold -C cyan

Notes
-----

- Time conversion assumes the trajectory frame spacing given by `dt`.
- Default `dt = 1e-4` corresponds to 0.1 ns between frames (typical SIRAH output).
- Figures are saved in PNG format by default but can be renamed.
